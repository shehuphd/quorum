...
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 openssl libncurses6 locales ca-certificates python3 python3-pip \
  && rm -rf /var/lib/apt/lists/*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/quorum ./

# The AI sidecar runs in this same container, on localhost, beside the release.
# Its Python deps install system-wide as root; the service and start script are
# copied in. Keys never enter the image: the start script writes keys.toml from
# an env var at runtime, and .dockerignore keeps project/ out of the context.
COPY sidecar ./sidecar
COPY bin/aca-start.sh ./aca-start.sh
RUN pip3 install --break-system-packages --no-cache-dir -r sidecar/requirements.txt \
  && chmod +x ./aca-start.sh
