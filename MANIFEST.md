...
| File | Role |
|---|---|
| `lib/quorum/ai.ex` | The `Quorum.AI` domain and the one door to the sidecar: `enabled?/0` for every feature to gate on, `generate/3` and `generate_json/3` which make the call and record what it spent, `calls/1`, `spend/0`, and `clear_spend/0` for the counter, and `targets/0`, `providers/0`, `add_key/1`, `pin_model/2`, `remove_key/1` behind the API keys settings tab. Quorum holds no provider keys and never speaks to a provider. Two rolling-day ceilings gate every call, `daily_budget/0` in dollars and `daily_call_limit/0` in calls, because the ledger can't price a model it doesn't know and an unpriced call would otherwise cost nothing against a dollar limit. |
| `lib/quorum/ai/call.ex` | `Call` resource: one model call as a record. Purpose, provider, model, tokens, dollars where the rates ledger prices the model, elapsed time, and whether it worked, written on success and failure alike. Plain ids rather than foreign keys, so the spend record outlives the room it was spent on. Postgres table `ai_calls`. |
...
| `lib/quorum/ai/draft_job.ex` | Drafts a suggested answer when a question is spotlighted, presenter-only: two or three bullet points, fifty words at most, each one a sentence the presenter could say aloud. The reply is trimmed to the bullets it asked for, and a wall of text is dropped rather than stored, so a model coaxed off script can't write into the notes. A question keeps its draft, so a re-spotlight bills nothing. |
...
| `lib/quorum_web/live/settings_live.ex` | `/host/:host_token/settings/:tab`. Seven categories on a rail: Room, Questions, Moderation, Readings, Projection, Appearance, API keys. The API keys tab manages the sidecar's key file: a pasted key is tested live before it's stored, shown as its first four characters and asterisks, pinned to a model from the provider's usable catalog, or removed; under it, the spend counter and its clear button. Every control saves on change, so there is no save button; the status line has its height reserved so the first save doesn't move the buttons under the pointer. All seven panes are drawn. Deleting a room needs the session closed and its name typed. |
| `lib/quorum_web/controllers/page_controller.ex`, `page_html.ex`, `page_html/*` | `/`, `/privacy`, `/accessibility`, `/contact`. The landing page plus the three standing pages the footer links to. The contact form is guarded by a honeypot, a per-session cooldown, and the server-side wait in `Quorum.Contact.Limit`. A sent message leaves the form in place and empty rather than replacing the page. |
...
| `lib/quorum/contact/limit.ex` | How often the contact form may be used, held in ETS rather than in the sender's own cookie: one message per address every five minutes, and twenty an hour across everyone. Counts the address the ingress recorded, the last entry of `X-Forwarded-For`, so neither clearing cookies nor forging the front of that list gets past it. State belongs to the instance, which is the whole picture at one replica. |
| `lib/quorum_web/shell.ex` | The site header and footer. Nav entries render only when the page behind them exists, so the header never offers a link that goes nowhere. Three variants: `:app` for a presenter's pages, `:guest` for sign-in, and `:student` in a room, where the only two places to go are home and another code. Also `join_hint/0`, the host students type, read from the endpoint so the console and the projection name wherever the app is being served from. |
...
| `lib/quorum/release.ex` | Release tasks for production, where Mix isn't installed: `migrate/0` runs pending migrations (called by `bin/aca-start.sh` on boot) and `rollback/2` reverses to a version. |
| `lib/quorum/mailer.ex` | Swoosh mailer, behind the contact form (and the dormant sign-in link). In development it writes to the local mailbox rather than sending. In production `config/runtime.exs` picks the adapter from what the environment carries: Brevo, Mailgun, or the log. |
...
| File | Role |
|---|---|
| `lib/mix/tasks/quorum.demo.ex` | `mix quorum.demo`. Opens a room, seeds eight questions with plausible vote counts and one already answered, and prints the join code with the student, host, and projection links. |
...
| `priv/repo/migrations/20260910084134_add_hall_to_room.exs` | Adds `rooms.projection_dark?`, which hall the projection is showing now. |
| `priv/repo/migrations/20260910091848_add_oban_jobs.exs` | Installs Oban's own tables (`Oban.Migration`), for the auto-close cron and the AI jobs. Leaves the table on the way down for anything still queued. |
| `priv/repo/migrations/20260910093008_cascade_questions_with_room.exs` | Makes `questions.room_id` cascade-delete, so a room with anything in it can be deleted. |
| `priv/repo/migrations/20260910122908_add_ai_calls.exs` | Creates `ai_calls`: one row per model call, with purpose, provider, model, tokens, dollars, elapsed time, and whether it worked. Plain ids, so the record outlives its room. |
| `priv/repo/migrations/20260910123201_add_question_ai_fields.exs` | Adds `questions.held_reason`, `pointer_reading_ids`, and `answer_draft`, and `rooms.hold_injection?`. |
| `priv/repo/migrations/20260910203808_default_room_injection_screen.exs` | Flips `rooms.hold_injection?` to default true and turns it on for existing rooms, so the open demo screens by default. |
...
| `Dockerfile` | The production image, from `mix phx.gen.release --docker` and then extended: a builder stage compiles the release, and the runtime stage carries Python and the sidecar's dependencies alongside it, so one container runs both processes. |
| `bin/aca-start.sh` | The container's entry point. Writes `project/keys.toml` from the `QUORUM_KEYS_TOML` secret, starts the sidecar in the background on `127.0.0.1:4747`, runs the migrations, then hands the foreground to the Phoenix release. |
| `.dockerignore` | Keeps `project/`, `.env*`, `.neon`, `neon.ts`, and `node_modules` out of the build context, so no key or credential can reach an image layer. |
