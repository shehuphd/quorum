...
| `lib/quorum/contact.ex` | The contact form's validation and delivery. Mail goes from Quorum's own address with the sender on `reply-to`, so it doesn't fail SPF or DKIM at the receiving end. `deliver/1` returns an error for a mailer that raises or exits, which a missing provider does, so the page says the message didn't send rather than failing on it. |
