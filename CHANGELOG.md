...
- Six view helpers several screens defined identically (vote and question pluralisation, the wall clock, the asker line, the QR code, the hall gradient) move to one `QuorumWeb.Wording` module the screens import. No behaviour change; the shared definitions are what the screens already drew. An unused `.q-tab-note` CSS rule, which nothing rendered, is gone with them.
- Removed code nothing reached: the generated `QuorumWeb.CoreComponents` and the three unused `Layouts` functions (`app`, `flash_group`, `theme_toggle`), the `Reading` `:edit` action (no helper, UI, or test ever invoked it), and `Contact.Limit.per_sender_seconds/0`. The screens render their own components against the `q-*` design system, so the Phoenix scaffolding drew nothing; the root layout stays.
- The rooms index resolves each room's question count in one grouped query instead of one query per room, and the projection and join screens count a room's questions in the database rather than loading every row (with its vote aggregate) to call `length` on it. `Sessions.count_questions/1` and `list_rooms_with_counts/1` back these.
- The console and projection no longer print the deployment's raw host URL as prose. The joining panel says "Students join with" and the strip "Still joining", each above the code, with the "Copy student link" button carrying the URL. The projection leads with the code and keeps a faint "or go to <host>/join to type it in" as a quiet fallback for anyone who can't scan. With the long URL gone, the panel's action buttons align under the code. The sign-in page drops the single sign-on explainer line, and the empty console reads just "Questions appear here the moment a student posts one."
- A maker's credit, "By Mo Shehu", centred at the foot of every page (site shell, sign-in, join, projection), linking to mohammedshehu.com.
- The landing hero's join code and the reading-pointer card cycle on their own. The code steps through random five-character strings; the card steps through a set of philosophy scenarios, question and readings together. Both hold still under reduced-motion and pause when the tab is hidden.
- The demo band drops the "The presenter's console, with a session already running." caption, which the "See demo" button already implies, and the setup line no longer trails off into how retention reads back.
- `Quorum.Heartbeat` runs a `SELECT 1` every four minutes while the app is up, so Neon doesn't suspend underneath a running container during a lull in a session. Off by default, on in production. (A daily warm-window scale rule was tried and dropped: holding a replica up across demo hours runs past the free tier, and the app is meant to cost nothing between demos. It stays on scale-to-zero, waking on the first request.)
- Migrations run inside the release's own boot now (`QUORUM_MIGRATE_ON_BOOT`, carried out by `Quorum.Application.start/2`) rather than in a separate `bin/migrate` node before it. One BEAM start per container instead of two, which takes seconds off a cold boot.
- The sidecar pins `keycall>=1.12.0`. The floor was `>=1.11.1`, and because the pip layer caches on an unchanged `requirements.txt`, the deployed sidecar had stayed on the KeyCall that was current when the layer first built rather than tracking new releases. Raising the floor busts that layer, so the build resolves 1.12.0 and the deployed sidecar runs it. Verified against the sidecar's imports, including the private `keycall._registry` catalog it reads.
