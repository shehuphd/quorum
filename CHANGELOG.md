...
- Cold starts are cut on two fronts. A cron scale rule holds one replica up daily 08:00–20:00 London, the hours a demo is likely, so a request in that window meets a warm app; an HTTP rule still wakes it from zero outside the window. `Quorum.Heartbeat` runs a `SELECT 1` every four minutes while the app is up, which keeps Neon from suspending underneath a held replica. Off by default, on in production.
