...
- Production picks its mail adapter at boot: Mailgun when `MAILGUN_API_KEY` and `MAILGUN_DOMAIN` are set, and the log otherwise, so a deployment with no mail credentials still takes the form. `CONTACT_EMAIL` sets where it goes.
