...
- AI provider calls through a localhost Python sidecar built on KeyCall (`sidecar/`): Quorum holds no provider keys, speaks HTTP to one normalized `/generate` plus a small key-management API the settings screen drives, and records every call's spend in `ai_calls`
