...
A new room starts with twenty held words so it isn't ungated on day one; the list is a starting point a presenter edits, not a policy. The triggers feed that one decision, resolved by `Sessions.hold_reason/3`: the room holds everything, the asker has had nothing approved in this room, the body uses a held word, or the body carries a link. Students have no accounts, so the second reads trust per room from what that browser has had approved in it. Every trigger holds and none refuses, which keeps the cost of a false positive to a wait and means the triggers can be blunt without being punitive.

Because the AI reads posted questions (to point a student at a reading, to draft the presenter a suggested answer), a question that's really an instruction to the model is its own kind of abuse. Two things guard it, both before a model acts. Every body is normalized on the way in, so zero-width and bidi characters can't hide the wording. Then `Sessions.Injection` holds the blatant attempts deterministically, with no model call and whatever the room screens, whenever the AI is reachable; a per-room model screen, on by default, reads the fuzzier cases and holds what it flags. The reading pointer only ever picks from the approved list and the draft is the presenter's to read alone, so a screen a jailbreak slips past still can't reach the hall on its own.

Questions outlive the session. A term of them is the record of which material missed, which is what a room is kept for rather than a default nobody chose, so retention is on unless a room turns it off. A room that does has its questions and votes deleted when the session closes.
...
| A question is really an instruction aimed at the AI | Two layers, both before a model acts on the text. Every posted body is normalized first, so zero-width and bidi characters can't hide the wording from a match or a reader. `Sessions.Injection` then holds the blatant attempts (the "ignore previous instructions" opener, a demand for the system prompt, a smuggled chat-role marker) deterministically, with no model call and whatever the room screens, whenever the AI is reachable at all. The opt-in model screen behind `hold_injection?`, on by default, reads the fuzzy rest into `:injection`. Behind both, the reading pointer only picks from the approved list and the draft is the presenter's to read alone and trimmed to the bullets asked for, so a screen a jailbreak slips past still can't reach the hall on its own |
...
- Sign-in emails arrive in the local mailbox at `/dev/mailbox` in development, so the link is readable without a mail provider
